...
import {CLOUD_ASSETS_ROOT, CLOUD_IMG_PATH} from '../config';
import {IMG_TYPE} from "../constants";

var _FileCache = {};

export default {
  callFunction: (name, params) => new Promise((resolve, reject) => {
    const {event, data, success, fail} = params;
    ...
    if(!filePath.startsWith('http://tmp/')) {
      ...
      Taro.getFileInfo({filePath, digestAlgorithm: 'md5'}).then(info => {
        ...
        if(_FileCache[fileName]) {
          ...
          success && success(i, {id: _FileCache[info.digest], fileName: fileName, url: CLOUD_ASSETS_ROOT + path + fileName});
          next();
        } else {
          if(Taro.getEnv() == Taro.ENV_TYPE.WEAPP) {
            Taro.cloud.uploadFile({
              ...
              success: (resp) => {
                console.log(fileName);
                success && success(i, {id: resp.fileID, fileName: fileName, url: CLOUD_ASSETS_ROOT + path + fileName});
